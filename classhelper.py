...
# 创建一个函数，用于确认是否关闭窗口
def confirm_close():
    result = tkinter.messagebox.askquestion("确认关闭", "确定要关闭窗口吗？")
    if result == "yes":
        root.destroy()


# 创建右键"隐藏"按钮
hide_button = tk.Button(root, text="隐藏", command=hide_window, relief=tk.FLAT, bg="#FFFFFF", width=6, height=1)
hide_button.place(x=5, y=240)

# 创建 "关闭" 按钮
close_button = tk.Button(root, text="关闭", command=confirm_close, relief=tk.FLAT, bg="#FFFFFF", width=6, height=1)
close_button.place(x=70, y=240)
